...
PHP;
    ...
    public static function dumpVersionsClass(Event $composerEvent)
    {
        ...
        if (!array_key_exists('muglug/package-versions', $versions)) {
    ...
    private static function writeVersionClassToFile($versionClassSource, Composer $composer, IOInterface $io)
    {
        ...
        if (! file_exists(dirname($installPath))) {
            $io->write('<info>muglug/package-versions:</info> Package not found (probably scheduled for removal); generation of version class skipped.');

            return;
        }

        $io->write('<info>muglug/package-versions:</info>  Generating version class...');

        file_put_contents($installPath, $versionClassSource);
        chmod($installPath, 0664);

        $io->write('<info>muglug/package-versions:</info> ...done generating version class');
    }
    ...
    private static function locateRootPackageInstallPath(
        ...
        if ('muglug/package-versions' === self::getRootPackageAlias($rootPackage)->getName()) {
            return dirname($composerConfig->get('vendor-dir'));
        }

        return $composerConfig->get('vendor-dir') . '/muglug/package-versions';
    }
